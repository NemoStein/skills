...
node_script="$script_dir/../../isolate/scripts/worktree.mjs"
if [[ ! -f "$node_script" ]]; then
  printf '%s\n' 'Missing isolate lifecycle runtime. Install isolate and integrate together from the same skills.sh source.' >&2
  exit 1
fi
arguments=(
  "$node_script"
