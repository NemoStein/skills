...
Consult the [lifecycle contract](REFERENCE.md) when recovering from failure or changing package behavior; the steps below are the execution path. `isolate` owns the shared runtime used by the companion `integrate` and `discard` skills.
